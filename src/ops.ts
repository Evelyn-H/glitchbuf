...
const OPS: OpDef[] = [
  ...
  {
    name: 'reverb', desc: 'Freeverb-style reverb.', params: [
  ...
  {
    name: 'rescale', desc: 'resize the image. omit height to preserve aspect ratio.', params: [
  ...
  {
    name: 'resize', desc: 'resize the image. omit height to preserve aspect ratio. (alias for rescale)', params: [
      { name: 'width', type: ParamType.int, min: 64, max: 4096, default: 1024, step: 1, unit: 'px' },
      { name: 'height', type: ParamType.int, min: 64, max: 4096, default: 1024, step: 1, unit: 'px' },
    ], invoke: (buf, w, h) => buf.rescale(w, h)
  },
  ...
  {
    name: 'copy', desc: 'copy a region to another position in the buffer.', params: [
  ...
  {
    name: 'tremolo', desc: 'oscillate amplitude over the buffer length.', params: [
  ...
  {
    name: 'distort', desc: 'tanh waveshaper distortion.', params: [
      { name: 'drive', type: ParamType.float, min: 1, max: 10, default: 2, step: 0.1 },
    ], invoke: (buf, d) => buf.distort(d)
  },

  {
    name: 'chorus', desc: 'chorus/flanger modulation effect.', params: [
  ...
  {
    name: 'pitchshift', desc: 'shift pitch by semitones.', params: [
      { name: 'semitones', type: ParamType.float, min: -24, max: 24, default: 1, step: 0.1 },
    ], invoke: (buf, s) => buf.pitchShift(s)
  },

  {
    name: 'phaser', desc: 'all-pass phaser effect.', params: [
  ...
  {
    name: 'freqshift', desc: 'shift all frequency components by a fixed offset.', params: [
  ...
  {
    name: 'vibrato', desc: 'frequency modulation vibrato.', params: [
  ...
  {
    name: 'chebyshev', desc: 'chebyshev waveshaper — adds upper harmonics. positive = odd orders, negative = even orders.', params: [
      { name: 'order', type: ParamType.int, min: -20, max: 20, default: 1, step: 1 },
    ], invoke: (buf, o) => buf.chebyshev(o)
  },

  {
    name: 'autowah', desc: 'envelope-following auto-wah filter.', params: [
  ...
  {
    name: 'feedbackdelay', desc: 'feedback delay line.', params: [
  ...
  {
    name: 'sort', desc: 'pixel-sort horizontally by brightness threshold.', params: [
      { name: 'threshold', type: ParamType.float, min: 0, max: 100, default: 50, step: 1, unit: '%' },
    ], invoke: (buf, t) => buf.sort(t as Percentage)
  },

  {
    name: 'sortvertical', desc: 'pixel-sort vertically by brightness threshold.', params: [
      { name: 'threshold', type: ParamType.float, min: 0, max: 100, default: 50, step: 1, unit: '%' },
    ], invoke: (buf, t) => buf.sortvertical(t as Percentage)
  },

  {
    name: 'smear', desc: 'horizontal pixel smear / motion blur.', params: [
  ...
  {
    name: 'xor', desc: 'XOR each byte with a constant value.', params: [
      { name: 'value', type: ParamType.int, min: 0, max: 255, default: 85, step: 1 },
    ], invoke: (buf, v) => buf.xor(v)
  },

  {
    name: 'transpose', desc: 'shift one RGB channel by a pixel offset.', params: [
  ...
  {
    name: 'invert', desc: 'invert (negate) every byte in the buffer.', params: [],
    invoke: (buf) => buf.invert()
  },

  {
    name: 'shuffle', desc: 'randomly swap a fraction of pixels.', params: [
      { name: 'amount', type: ParamType.float, min: 0, max: 100, default: 50, step: 1, unit: '%' },
    ], invoke: (buf, pct) => buf.shuffle(pct as Percentage)
  },

  {
    name: 'quantize', desc: 'reduce colour to N evenly-spaced levels per channel.', params: [
      { name: 'levels', type: ParamType.int, min: 2, max: 256, default: 8, step: 1 },
    ], invoke: (buf, n) => buf.quantize(n)
  },

  {
    name: 'fold', desc: 'waveform-folding distortion.', params: [
      { name: 'drive', type: ParamType.float, min: 0.5, max: 5, default: 1.5, step: 0.1 },
    ], invoke: (buf, d) => buf.fold(d)
  },

  {
    name: 'solarize', desc: 'invert bytes above threshold (solarise effect).', params: [
